//10. Метод «бульбашки», Метод Хоара, Метод Шелла
#include <iostream>
#include <vector>
#include <algorithm>
 ...
 * 
 * Ідея алгоритму полягає в переставлянні елементів масиву таким чином, щоб його можна було розділити 
 ...
 Алгоритм швидкого сортування може бути реалізований як у масиві, так і в двозв'язному списку.

 .............................
 Число операций перемены местоположений элементов внутри массива значительно сократится,
 если менять местами далеко стоящие друг от друга элементы.
 Для этого выбирается для сравнения один элемент х, отыскивается слева первый элемент,
 который не меньше х, а справа первый элемент, который не больше х.
 Найденные элементы меняются местами. После первого же прохода все элементы, 
 которые меньше х, будут стоять слева от х, а все элементы, которые больше х, - справа от х. 
 С двумя половинами массива поступают точно также. 
 ...
 * 
 * 
 * Під час сортування вставкою елементи переміщуються лише на одну позицію за раз.
 Сортування оболонки ділить масив на менші частини на основі значення інтервалу та виконує сортування вставкою на цих частинах.

Поступово значення інтервалу зменшується, а розмір розділених частин збільшується. 
Оскільки фрагменти попередньо сортуються окремо, об’єднання цих фрагментів вимагає менше кроків, ніж сортування вставки.
 */
